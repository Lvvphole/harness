...
def _python_public_declarations(lines: list[str]) -> set[str]:
    names = set()
    for line in lines:
        match = PYTHON_DECLARATION_RE.match(line)
...
def grows_public_exports(proposal: dict[str, Any], worktree: Path | None = None) -> bool:
    ...
    for edit in proposal.get("edits", []):
        ...
        if Path(edit.get("path") or "").suffix == ".py":
            if _python_public_declarations(added) - _python_public_declarations(removed):
                return True
