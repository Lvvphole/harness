...
def _gate(payload: dict[str, Any], contract: dict[str, Any]) -> tuple[bool, str, list[str]]:
    ...
    if name == "apply_patch":
        ...
        if paths_in_scope(paths, allowed_paths):
    ...
    if name in READ_TOOLS:
        ...
        try:
            ...
            return False, str(exc), []
        return (False, "read path out of scope", paths) if paths_in_scope(paths, allowed_paths) else (True, "ok", [])
...
def _post_tool(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        pass
    request = {key: payload.get(key) for key in ("tool_name", "tool_use_id", "tool_input")}
    provenance = "PASS" if proposal and proposal.get("proposal_sha256") == _canonical_hash(request) else "FAIL"
    record = {
    ...
    if proposal and provenance == "FAIL":
