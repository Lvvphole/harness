...
def _load_contract(payload: dict[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    schema = (
        PLUGIN_ROOT / "skills" / "bounded-runtime-harness" /
        "assets" / "schemas" / "contract.schema.json"
    )
...
def _resolved_paths_out_of_scope(
    paths: list[str], allowed: list[str], cwd: Path,
) -> list[str]:
    ...
    roots: list[Path] = []
    for raw in allowed:
        candidate = repo if raw == "." else (cwd / raw).resolve(strict=False)
        if _is_within(repo, candidate):
    ...
    for raw in paths:
        ...
        if not _is_within(repo, target) or not any(
            _is_within(root, target) for root in roots
        ):
            out.append(raw)
    return out
...
def _gate(payload: dict[str, Any], contract: dict[str, Any]) -> tuple[bool, str, list[str]]:
    ...
    if name == "Bash":
        ...
        ok = (
            "run_command" in contract["allowed_tools"]
            and command in contract["allowed_commands"]
        )
    ...
    if name == "apply_patch":
        ...
        if _resolved_paths_out_of_scope(paths, allowed_paths, cwd):
        ...
        failure = _predicate_failure(proposal, contract, cwd, prior)
        if failure:
    ...
    if name in READ_TOOLS:
        ...
        try:
            ...
            return False, str(exc), []
        if _resolved_paths_out_of_scope(paths, allowed_paths, cwd):
            return False, "read path out of scope", paths
        return True, "ok", []
...
def _post_tool(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    provenance = (
        "PASS"
        if proposal and proposal.get("proposal_sha256") == _canonical_hash(request)
        else "FAIL"
    )
    record = {
...
def _lifecycle(event: str, payload: dict[str, Any]) -> dict[str, Any]:
    if event == "UserPromptSubmit":
        ...
        _write_state(payload, state)
    marker = {
        "information_state": "OBSERVED", "event": event,
        "session_id": payload.get("session_id"), "turn_id": payload.get("turn_id"),
    }
    name = (
        f"{event}-"
        f"{_safe_component(payload.get('turn_id') or payload.get('source') or payload.get('reason'))}.json"
    )
    try:
